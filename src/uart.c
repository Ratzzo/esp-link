 ...
 * ----------------------------------------------------------------------------
 * Some changes by Rtzz0 in 2018
 * - added external muxing with 74hc125
 */
#include "esp8266.h"
#include "task.h"
#include "uart.h"
...
volatile uint8_t _uart_mux_target = 0; //which target we want to send the data thru uart0
...
uart_config(uint8 uart_no, UartBaudRate baudrate, uint32 conf0)
{
  if (uart_no == UART1) {
  ...
  if (uart_no == UART0) {
	...
	SET_PERI_REG_MASK(UART_INT_ENA(uart_no), UART_RXFIFO_FULL_INT_ENA | UART_RXFIFO_TOUT_INT_ENA /*| UART_TXFIFO_EMPTY_INT_ENA*/);
	// when txfifo is empty the
  } else {
    WRITE_PERI_REG(UART_CONF1(uart_no),
...
volatile uint8_t _uart0_lastmuxdir = 0;
volatile uint8_t _uart0_muxdir = 0;

void ICACHE_FLASH_ATTR uart0_mux_select(int i){
	switch(i){
	case 0:
		gpio_output_set((1 << MUXPIN), 0, (1 << MUXPIN), 0);
		break;
	case 1:
		gpio_output_set(0, (1 << MUXPIN), (1 << MUXPIN), 0);
		break;
	}
}


// printf on specified uart0 multiplexer
// if the mux channel selecter is different on subsequent call, this will block until all previous data is transmitted
void ICACHE_FLASH_ATTR uart0_muxed_printf(int muxdir, const char *fmt, ...){
	char buff[100];
	int len;
	int i = 0;
	va_list args;
	va_start (args, fmt);
	ets_vsnprintf(buff, sizeof(buff), fmt, args); //printf to buff
	len = ets_strlen(buff);
	_uart0_muxdir = muxdir; //request muxdir change to interrupt handler
	//if muxdir is not the last muxdir used, and the transfer hasn't finished yet, then wait
	while((((READ_PERI_REG(UART_STATUS(UART0))>>UART_TXFIFO_CNT_S)&UART_TXFIFO_CNT)) && (_uart0_lastmuxdir != _uart0_muxdir)){

	}
	_uart0_lastmuxdir = _uart0_muxdir;
	i = system_get_cpu_freq() == SYS_CPU_80MHZ ? 200 : 400; //this should be ajusted according to the baud rate
	while(i--){
		__asm("");
	}
	uart0_mux_select(_uart0_muxdir);
	i = 0;

	while(len--) //put chars into tx fifo accordingly
...
*******************************************************************************/
void ICACHE_FLASH_ATTR
uart_init(uint32 conf0, UartBaudRate uart0_br, UartBaudRate uart1_br)
{
